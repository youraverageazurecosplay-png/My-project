...
def check_for_updates():
    ...
    try:
        ...
        if not messagebox.askyesno("Update", f"New version {remote_version} available. Download now?"):
            return

        code = requests.get(remote_script_url, timeout=5)
        if code.status_code == 200:
            with open("Gaming_Stuffs_new.py", "wb") as f:
                f.write(code.content)
            messagebox.showinfo(
                "Update",
                "Downloaded Gaming_Stuffs_new.py.\nClose this app and run the new file next time."
            )
        else:
            messagebox.showerror("Update", "Failed to download the update.")
    except Exception:
        messagebox.showinfo("Update", "Could not check for updates (offline or network error).")
