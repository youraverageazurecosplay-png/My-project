...
# Base directory = folder this script is in (portable for any user)
...
# ========= Games launchers (portable) ==========
def open_forsaken_practice():
    # Allow custom path from settings.json, else use local script
...
def open_roblox():
    ...
    if custom and os.path.exists(custom):
        # e.g. "/Applications/Roblox.app"
        cmd = ["/usr/bin/open", custom]
    else:
        # Fallback: just "open Roblox" by name; user can adjust via settings.json
        cmd = ["/usr/bin/open", "-a", "Roblox"]
